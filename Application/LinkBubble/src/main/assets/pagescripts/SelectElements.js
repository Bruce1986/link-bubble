...
(function () {
  ...
  function maskSelects () {
    ...
    Array.prototype.forEach.call(els, function (select) {
      ...
      if (!mask || mask.className !== '__link_bubble__select_mask__') {
        ...
        for (const i in positioningProps) {
          const prop = positioningProps[i];
  ...
  const observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      let changed = false;
      const allChangedNodes = [].slice.call(mutation.addedNodes).concat([].slice.call(mutation.removedNodes));
